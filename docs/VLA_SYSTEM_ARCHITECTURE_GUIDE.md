...
# 22. VLA-07 실제 runtime 배치

Ubuntu/Jazzy PC는 VLA Core, Mock/Qwen, Perception Bridge, UI를 실행한다. Debian 13 Raspberry Pi host에는 ROS가 없고 Docker host 역할만 한다. `IntelPi` Ubuntu 22.04/Humble Docker가 Hardware, SLAM, Nav2, `uncc_example`, VLA Navigation Bridge를 실행한다.

Pi container는 host network, privileged, `/dev` bind이며 Humble Nav2와 SLAM Toolbox가 설치되어 있다. Robot package는 Pi/Humble에서 build/run하므로 PC/Jazzy Nav2 설치는 필요하지 않다.

Persistent 설정을 바꾸지 않고 PC process만 domain 205 + Fast DDS로 실행하여 PC↔Pi String 및 `/vla/*` 양방향 transport를 확인했다.

Stationary 실측에서 odom, scan, map, `map → odom → base_footprint`, Nav2 lifecycle/action server, VLA Navigation Bridge actual pose, PC WorldModel 반영, board driver와 four-motor zero가 PASS했다.

Board driver는 launch chain에 포함되며 Docker `ubuntu` 사용자 runtime에서 실행해야 한다. Physical E-stop은 `PHYSICAL_ESTOP_NOT_IMPLEMENTED`다. Software halt contract는 goal cancel → cmd_vel stop → explicit four-motor zero다.

첫 short navigation은 **SAFE ABORT**였다. Expected `NAVIGATE_TO person_0001` 대신 Mock backend가 `RETURN_HOME`을 제안했고 stale robot pose Validator가 차단했다. 실제 Nav2 goal, non-zero cmd_vel, Robot movement는 모두 0이다.
