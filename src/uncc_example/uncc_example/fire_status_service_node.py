...
image_pipeline이 발행하는
/fire/detections JSON envelope를 구독해서
...
FIRE_DETECTION_TOPIC = '/fire/detections'
...
class FireStatusServiceNode(Node):
    ...
    def on_detections(self, msg: String):
        try:
            ...
            self.get_logger().warn(f'Invalid /fire/detections JSON: {exc}')
            return
        now = time.time()
        fire_detected = False
        best_score = 0.0

        for obj in payload.get('detections', []):
            if obj.get('class_name') == FIRE_CLASS_NAME:
                fire_detected = True
                best_score = max(best_score, float(obj.get('score', 0.0)))
