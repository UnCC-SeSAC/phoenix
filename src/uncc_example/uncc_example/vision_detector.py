...
class VisionDetector(Node):
    ...
    def __init__(self):
        ...
        # spin_thread=True 가 핵심이다. 이게 없으면 리스너가 메인 노드와
        # 같은 단일 스레드 executor 를 쓰는데, 아래 transform() 이 블로킹
        # 하는 동안 /tf 콜백이 못 돌아 buffer 가 영원히 안 채워진다.
        # 즉 변환이 준비 안 된 경우 timeout 을 통째로 소진하고 반드시
        # 실패하는 self-deadlock 이 된다 (감지 1건당 1초씩).
    ...
    def _compute_map_position(self, detection, stamp):
        ...
        try:
            ...
            self.get_logger().warn(
                ...
                throttle_duration_sec=2.0,  # 감지마다 찍혀 로그가 잠긴다
            )
            return None
