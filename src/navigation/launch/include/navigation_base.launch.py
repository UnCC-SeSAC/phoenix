...
def launch_setup(context):
    ...
    declare_use_teb_cmd = DeclareLaunchArgument("use_teb", default_value="false")

    if use_teb == "true":
        controller_param = os.path.join(
            navigation_package_path, "config/nav2_controller_teb.yaml"
        )
    else:
        controller_param = os.path.join(
            navigation_package_path, "config/nav2_controller_dwb.yaml"
        )
