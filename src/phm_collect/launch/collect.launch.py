...
def generate_launch_description():
    ...
    args = [
        ...
        # ★ 이 브랜치(albitro/phm_feat)에는 릴레이가 없습니다 — 기본 false.
        # controller 패키지에 rf2o_covariance_relay entry point 가 없어서
        # true 로 두면 런치 전체가 실패합니다. 릴레이와 그 짝인 ekf.yaml 변경은
        # albitro/data_dashboard 에만 있고 여기로 가져오지 않았습니다.
    ...
    #
    # ★ 이 브랜치에는 이 실행 파일이 없습니다 (use_relay 기본 false).
    # 끄면 아래 EKF 의 odom1(= odom_rf2o_fixed) 입력이 안 옵니다 — EKF 는
    # odom0(휠) + imu0 으로만 돕니다. PHM 분석은 /odom_rf2o 원본과 자이로를
    # 직접 읽으므로 영향이 없습니다.
    ...
    return LaunchDescription(args + [
        ...
        GroupAction(condition=IfCondition(LaunchConfiguration('use_rf2o')),
                    actions=[rf2o_node]),
        GroupAction(condition=IfCondition(LaunchConfiguration('use_relay')),
                    actions=[rf2o_relay_node]),
