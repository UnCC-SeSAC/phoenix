...
| `rf2o_covariance_relay` | `/odom_rf2o_fixed` | **이 브랜치에 없습니다** — 아래 참고 |
...
### ★ `rf2o_covariance_relay` 는 이 브랜치(albitro/phm_feat)에 없습니다

릴레이 실행 파일(`controller` 패키지)과 그 짝인 `ekf.yaml` 의 `odom1` 변경은
`albitro/data_dashboard` 에만 있고 여기로 가져오지 않았습니다. 그 둘은 **EKF 의 위치
추정을 바꾸는 변경**이라 주행 거동에 영향을 주고, PHM 과는 검토 관점이 다릅니다.

**PHM 은 릴레이 없이 완전히 동작합니다.** 감시 노드는 원본 `/odom_rf2o` 를 구독하고
부호를 스스로 보정합니다(`phm_detect_core.AXES['fwd']['sign'] = -1.0`).
`twist.linear.x` 하나만 쓰고 covariance 는 보지 않기 때문입니다.

그래서 두 런치의 릴레이 인자는 **기본이 꺼짐**입니다. 켜면 entry point 가 없어 런치가
실패합니다.

    phm_monitor.launch.py   with_relay:=false   (기본)
    collect.launch.py       use_relay:=false    (기본)

`collect.launch.py` 에서 끄면 EKF 의 `odom1`(= `odom_rf2o_fixed`) 입력이 안 옵니다 —
EKF 는 `odom0`(휠) + `imu0` 으로만 돕니다. 잔차 분석은 `/odom_rf2o` 원본과 자이로를
직접 읽으므로 영향이 없습니다.

나중에 릴레이까지 이식하면 두 인자를 `true` 로 켜세요.
