...
class LidarController(Node):
    def __init__(self, name):
        ...
        self.create_service(
            SetInt64, "~/set_running", self.set_running_srv_callback
        )  # 开启玩法(start the game)
        Heart(
            self,
            self.name + "/heartbeat",
        ...
        self.create_service(
            SetFloat64List, "~/set_param", self.set_parameters_srv_callback
        )  # 参数设置(set parameter)
    ...
    def enter_srv_callback(self, request, response):
        ...
        self.lidar_sub = self.create_subscription(
            LaserScan, "/scan_raw", self.lidar_callback, qos
        )  # 订阅雷达数据(subscribe to Lidar data)
    ...
    def set_running_srv_callback(self, request, response):
        ...
        self.get_logger().info(
            "\033[1;32m%s\033[0m" % ("set_running " + str(new_running_mode))
        )
    ...
    def set_parameters_srv_callback(self, request, response):
        ...
        if not 0.3 <= new_threshold <= 1.5:
            response.success = False
            response.message = (
                "New threshold ({:.2f}) is out of range (0.3 ~ 1.5)".format(
                    new_threshold
                )
            )
            return response
    ...
    def lidar_callback(self, lidar_data):
        ...
        if self.lidar_type != "G4":
            max_index = int(
                math.radians(MAX_SCAN_ANGLE / 2.0) / lidar_data.angle_increment
            )
            left_ranges = lidar_data.ranges[:max_index]  # 左半边数据 (the left data)
            right_ranges = lidar_data.ranges[::-1][
                :max_index
            ]  # 右半边数据 (the right data)
            ...
            """
            min_index = int(
                math.radians((360 - MAX_SCAN_ANGLE) / 2.0) / lidar_data.angle_increment
            )
            max_index = min_index + int(
                math.radians(MAX_SCAN_ANGLE / 2.0) / lidar_data.angle_increment
            )
            left_ranges = lidar_data.ranges[::-1][min_index:max_index][
                ::-1
            ]  # 左半边数据 (the left data)
            right_ranges = lidar_data.ranges[min_index:max_index][
                ::-1
            ]  # 右半边数据 (the right data)
        ...
        with self.lock:
            ...
            left_range, right_range = np.array(left_ranges[:angle_index]), np.array(
                right_ranges[:angle_index]
            )
            if self.machine_type != "MentorPi_Acker":
                if self.running_mode == 1 and self.timestamp <= time.time():
                    ...
                    if len(min_dist_left_) > 1 and len(min_dist_right_) > 1:
                        ...
                        if (
                            min_dist_left <= self.threshold
                            and min_dist_right > self.threshold
                        ):  # 左侧有障碍(there is obstacle at left)
                            twist.linear.x = (
                                self.speed / 6
                            )  # 转弯时向前速度尽量小(keep the forward velocity as low as possible when turning)
                            max_angle = math.radians(90)  # 转90度(turn 90 degree)
                            w = (
                                self.speed * 6.0
                            )  # 转弯速度尽量大(keep the turning speed as high as possible)
                            twist.angular.z = -w
                            ...
                            min_dist_left > self.threshold
                            and min_dist_right <= self.threshold
                        ):  # 右侧有障碍(there is obstacles at right)
                            twist.linear.x = (
                                self.speed / 6
                            )  # 转弯时向前速度尽量小(keep the forward velocity as low as possible when turning)
                    ...
                    if len(dist_) > 0:
                        ...
                        angle = (
                            -angle + lidar_data.angle_increment * min_index
                        )  # 计算最小值对应的角度(calculate the angle corresponding to the minimum value)
                        # self.get_logger().info(str(dist_))
                        # self.get_logger().info(str([dist, angle]))
                        if (
                            dist < self.threshold and abs(math.degrees(angle)) > 5
                        ):  # 控制左右(control the left and right)
                            if self.lidar_type != "G4":
                        ...
                        if (
                            dist < self.threshold and abs(0.2 - dist) > 0.02
                        ):  # 控制前后(control the front and back)
                            self.pid_dist.update(self.threshold / 2 - dist)
                            twist.linear.x = common.set_range(
                                self.pid_dist.output, -self.speed, self.speed
                            )
                        else:
                            self.pid_dist.clear()
                ...
                if self.running_mode == 1 and self.timestamp <= time.time():
                    ...
                    if len(dist_) > 1:
                        ...
                        if (
                            dist < 2 and abs(0.5 - dist) > 0.02
                        ):  # 控制前后(control the front and back)
                            self.pid_dist.SetPoint = 0.5
                            self.pid_dist.update(dist)
                            twist.linear.x = common.set_range(
                                -self.pid_dist.output, -0.35, 0.35
                            )
                        else:
                            self.pid_dist.clear()
                        if (
                            dist < 2 and abs(math.degrees(angle)) > 5
                        ):  # 控制左右(control the left and right)
                            if self.lidar_type != "G4":
                                self.pid_yaw.update(-angle)
                                twist.angular.z = (
                                    ...
                                    * math.tan(
                                        common.set_range(
                                            self.pid_yaw.output, -0.316, 0.316
                                        )
                                    )
                                    / 0.145
                                )
